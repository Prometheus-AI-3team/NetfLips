...
def main():
    parser = argparse.ArgumentParser(description="Extract lip region using YOLOv8 face detection + Landmarks.")
    ...
    with tqdm(total=total_frames) as pbar:
        while True:
            ...
            # Fallback if no one is clearly talking (low variance)
            if winner_id is not None and max_var < 0.01:
                 # Pick the one with the highest current MAR (as a secondary heuristic)
                 winner_id = max(current_frame_mar, key=current_frame_mar.get) if current_frame_mar else winner_id
            ...
            if best_bbox_coords is not None:
                coords_list.append(best_face_bbox.tolist())
                out_frame = crop_and_resize(frame, best_bbox_coords, (96, 96))
            else:
                coords_list.append([0.0, 0.0, 0.0, 0.0])
